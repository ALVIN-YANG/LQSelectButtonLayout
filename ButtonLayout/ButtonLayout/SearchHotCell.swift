...
protocol SearchHotCellDelegate {
  
  func searchHotCellHeight(_ height: CGFloat)
}
class SearchHotCell: UITableViewCell {
  
  var elements = [""]
  
  
  var delegate: SearchHotCellDelegate?
...
extension SearchHotCell: UICollectionViewDelegate {
  
}

extension SearchHotCell: UICollectionViewDataSource {
...
extension SearchHotCell: FlexibleLayoutDelegate {
  ...
  func collectionViewContaintHeight(_ height: CGFloat) {
    delegate?.searchHotCellHeight(height)
  }
}
